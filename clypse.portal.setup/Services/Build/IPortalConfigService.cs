...
{
    /// <summary>
    /// Generates a configured portal settings file based on a template, without deployment metadata.
    /// </summary>
    ...
    /// <summary>
    /// Generates a configured portal settings file based on a template, including deployment metadata.
    /// </summary>
    /// <param name="templatePath">Path to the JSON template file.</param>
    /// <param name="s3DataBucketName">S3 bucket name that stores user data.</param>
    /// <param name="s3Region">AWS region for the S3 bucket.</param>
    ...
    /// <param name="cognitoRegion">AWS region for Cognito resources.</param>
    /// <param name="cognitoIdentityPoolId">Cognito identity pool identifier.</param>
    /// <param name="deployedBy">GitHub username of the user who deployed the build.</param>
    /// <param name="deployedAt">Date and time when the build was deployed.</param>
    /// <param name="deploymentActionUrl">URL to the GitHub Actions workflow run.</param>
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    /// <returns>A memory stream containing the configured JSON content.</returns>
    public Task<MemoryStream> ConfigureAsync(
        string templatePath,
